...
numeric_cols = df.columns[:-3].drop("adjuvant_radiotherapy")  # 数值列
target_col = 'adjuvant_radiotherapy'
...
print(f"准确率 (Accuracy): {accuracy_score(y_test, y_pred_final):.4f}")
print(f"AUC: {roc_auc_score(y_test, y_proba_final):.4f}")
print(f"AUPRC: {average_precision_score(y_test, y_proba_final):.4f}")
