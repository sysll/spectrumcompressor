# Recurrence Prediction Model Project

This project contains multimodal deep learning models for recurrence prediction, along with related comparative experiments and ablation studies.

## Project Overview

This project aims to develop and evaluate multimodal deep learning models for recurrence prediction. Core innovations include:
...
6. **Ablation Studies**: Evaluating the contribution of each model component to performance

## Project Structure

```
recurrence prediction/
├── 传统的Bert的对比模型/       # Traditional machine learning model comparison experiments
...
├── 我们的Bert对比模型/         # BERT-based model comparison experiments
│   ├── 1.Catboost.py         # CatBoost classifier
│   ├── 2.XGBoost.py          # XGBoost classifier
│   ├── 3.AdaBoost.py         # AdaBoost classifier
│   ├── 4.神经网络.py         # Neural network classifier
...
```bash
# Navigate to ablation experiment directory
cd 消融实验
...
Contains 10 model variants to evaluate the contribution of different components:
...
- FFT compression without gating: Removes spectral gating mechanism
...
- Top-K pooling without learnable weights: Uses regular Top-K pooling, removes learnable weights
...
- Early stopping: Stops training when validation set Accuracy does not improve for 10 consecutive epochs
